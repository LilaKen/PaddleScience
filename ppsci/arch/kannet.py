...
class KANLinear(paddle.nn.Layer):
    def __init__(
        ...
        if enable_standalone_scale_spline:
            self.spline_scaler = paddle.base.framework.EagerParamBase.from_tensor(
                tensor=paddle.empty(shape=[out_features, in_features])
            )
    ...
    def reset_parameters(self):
        init_KaimingUniform = paddle.nn.initializer.KaimingUniform(
            negative_slope=math.sqrt(5) * self.scale_base, nonlinearity="leaky_relu"
        )
    ...
    def forward(self, x: paddle.Tensor):
        ...
        spline_output = paddle.nn.functional.linear(
            x=self.b_splines(x)
            .reshape([x.shape[0], -1])
            .contiguous(),  # .view(x.shape[0], -1),
            weight=self.scaled_spline_weight.reshape(
                [self.out_features, -1]
            ).T.contiguous(),
        )  # .view(self.out_features, -1).T)
        # 采用view似乎无法计算一阶微分
        # spline_output = paddle.nn.functional.linear(
    ...
    def update_grid(self, x: paddle.Tensor, margin=0.01):
        ...
        grid = paddle.concat(
            x=[
                grid[:1]
                - uniform_step
                * paddle.arange(start=self.spline_order, end=0, step=-1).unsqueeze(
                    axis=1
                ),
                grid,
                grid[-1:]
                + uniform_step
                * paddle.arange(start=1, end=self.spline_order + 1).unsqueeze(axis=1),
            ],
            axis=0,
        )
...
class KAN(paddle.nn.Layer):
    def __init__(
        ...
        self.integral_layer = paddle.nn.Linear(
            in_features=sample_number, out_features=1, bias_attr=None
        )
