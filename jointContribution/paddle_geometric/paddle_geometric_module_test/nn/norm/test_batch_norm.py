...
def test_hetero_batch_norm(device, conf):
    ...
    # Test multiple types:
    type_vec = paddle.randint(5, (100, ), place=device)
    ...
    for i in range(5):  # Check that mean=0 and std=1 across all types:
        ...
        assert paddle.allclose(mean, paddle.zeros_like(mean), atol=1e-7)
        assert paddle.allclose(std, paddle.ones_like(std), atol=1e-7)
