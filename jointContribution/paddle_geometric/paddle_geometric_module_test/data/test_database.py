...
def test_database_single_tensor(tmp_path, Database, batch_size):
    ...
    data_list = paddle.randn(shape=[2, 5])
...
def test_database_schema(tmp_path, Database):
    ...
    data1 = (1, 0.1, 'a', paddle.randn(shape=[2, 8]), Data(x=paddle.randn(shape=[8])))
    data2 = (2, float('inf'), 'b', paddle.randn(shape=[2, 16]), Data(x=paddle.randn(shape=[8])))
    data3 = (3, float('NaN'), 'c', paddle.randn(shape=[2, 32]), Data(x=paddle.randn(shape=[8])))
    db.insert(0, data1)
    ...
    data1 = {
        ...
        'float': 0.1,
        'str': 'a',
        'tensor': paddle.randn(shape=[2, 8]),
        'data': Data(x=paddle.randn(shape=[1, 8])),
    }
    data2 = {
        'int': 2,
        'float': 0.2,
        'str': 'b',
        'tensor': paddle.randn(shape=[2, 16]),
        'data': Data(x=paddle.randn(shape=[2, 8])),
    }
    data3 = {
        'int': 3,
        'float': 0.3,
        'str': 'c',
        'tensor': paddle.randn(shape=[2, 32]),
        'data': Data(x=paddle.randn(shape=[3, 8])),
    }
    db.insert(0, data1)
    db.multi_insert([1, 2], [data2, data3])
...
def test_database_syntactic_sugar(tmp_path):
    ...
    assert paddle.equal(paddle.stack(db[:3], dim=0), data[:3])
    assert paddle.equal(paddle.stack(db[3:], dim=0), data[3:])
    assert paddle.equal(paddle.stack(db[1::2], dim=0), data[1::2])
    assert paddle.equal(paddle.stack(db[[4, 3]], dim=0), data[[4, 3]])
    assert paddle.equal(
        paddle.stack(db[paddle.to_tensor([4, 3])], dim=0),
        data[paddle.to_tensor([4, 3])],
    )
    assert paddle.equal(
        paddle.stack(db[paddle.to_tensor([4, 4])], dim=0),
...
if __name__ == '__main__':
    ...
    data = paddle.randn(shape=[args.numel, 128])
