...
def test_in_memory_dataset():
    ...
    assert paddle.equal(dataset.x, paddle.concat([x1, x2], dim=0))
    assert dataset.edge_index.tolist() == [
        [0, 1, 1, 2, 10, 11, 11, 12],
        [1, 0, 2, 1, 11, 10, 12, 11],
    ]
    assert paddle.equal(dataset[1:].x, x2)
...
def test_stored_in_memory_dataset(tmp_path):
    ...
    assert paddle.equal(dataset[0].x, x1)
    assert paddle.equal(dataset[0].edge_index, edge_index)
    assert dataset[0].num_nodes == 3
    assert paddle.equal(dataset[0].test_int, paddle.to_tensor([1]))
    assert dataset[0].test_str == '1'

    assert paddle.equal(dataset[1].x, x2)
    assert paddle.equal(dataset[1].edge_index, edge_index)
    assert dataset[1].num_nodes == 4
    assert paddle.equal(dataset[1].test_int, paddle.to_tensor([2]))
    assert dataset[1].test_str == '2'
...
def test_in_memory_num_classes():
    ...
    dataset = MyTestDataset([
        Data(y=paddle.to_tensor([[0, 1, 0, 1]])),
        Data(y=paddle.to_tensor([[1, 0, 0, 0]])),
        Data(y=paddle.to_tensor([[0, 0, 1, 0]])),
    ])
...
def test_in_memory_dataset_copy():
    data_list = [Data(x=paddle.randn(shape=[5, 16])) for _ in range(4)]
    ...
    for copied_data, data in zip(copied_dataset, dataset):
        assert paddle.equal(copied_data.x, data.x)

    copied_dataset = dataset.copy([1, 2])
    assert len(copied_dataset) == 2
    assert paddle.equal(copied_dataset[0].x, data_list[1].x)
    assert paddle.equal(copied_dataset[1].x, data_list[2].x)
...
def test_in_memory_sparse_tensor_dataset():
    x = paddle.randn(shape=[11, 16])
    adj = SparseTensor(
        row=paddle.to_tensor([4, 1, 3, 2, 2, 3]),
        col=paddle.to_tensor([1, 3, 2, 3, 3, 2]),
        sparse_sizes=(11, 11),
    )
...
def test_hetero_in_memory_dataset():
    data1 = HeteroData()
    data1.y = paddle.randn(shape=[5])
    data1['paper'].x = paddle.randn(shape=[10, 16])
    data1['paper', 'paper'].edge_index = paddle.randint(0, 10, (2, 30)).long()

    data2 = HeteroData()
    data2.y = paddle.randn(shape=[5])
    data2['paper'].x = paddle.randn(shape=[10, 16])
