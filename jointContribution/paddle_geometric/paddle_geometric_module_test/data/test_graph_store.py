...
from paddle_geometric.testing import MyGraphStore, get_random_edge_index
...
def test_graph_store():
    ...
    csr = paddle.to_tensor([0, 1, 2]), paddle.to_tensor([1, 2])
    ...
    assert paddle.equal(graph_store['edge_type', 'coo'][0], coo[0])
    assert paddle.equal(graph_store['edge_type', 'coo'][1], coo[1])
    assert paddle.equal(graph_store['edge_type', 'csr'][0], csr[0])
    assert paddle.equal(graph_store['edge_type', 'csr'][1], csr[1])
    assert paddle.equal(graph_store['edge_type', 'csc'][0], csc[0])
    assert paddle.equal(graph_store['edge_type', 'csc'][1], csc[1])
...
def test_graph_store_conversion():
    ...
    adj = to_paddle_coo_tensor(edge_index, size=(100, 100))
    coo = (adj.indices()[0], adj.indices()[1])
    adj = to_paddle_csr_tensor(edge_index, size=(100, 100))
    csr = (adj.crow_indices(), adj.col_indices())
    adj = to_paddle_csc_tensor(edge_index, size=(100, 100))
    ...
    for row, col, perm in zip(row_dict.values(), col_dict.values(),
                              perm_dict.values()):
        assert paddle.equal(row.sort()[0], coo[0].sort()[0])
        assert paddle.equal(col.sort()[0], coo[1].sort()[0])
        assert perm is None
    ...
    for row, col in zip(row_dict.values(), col_dict.values()):
        assert paddle.equal(row, csr[0])
        assert paddle.equal(col.sort()[0], csr[1].sort()[0])

    # Convert to CSC:
    row_dict, col_dict, perm_dict = graph_store.csc()
    assert len(row_dict) == len(col_dict) == len(perm_dict) == 3
    for row, col in zip(row_dict.values(), col_dict.values()):
        assert paddle.equal(row.sort()[0], csc[0].sort()[0])
        assert paddle.equal(col, csc[1])

    # Ensure that 'edge_types' parameters work as intended:
    out = graph_store.coo([('v', '1', 'v')])
    assert paddle.equal(list(out[0].values())[0], coo[0])
    assert paddle.equal(list(out[1].values())[0], coo[1])
    ...
    assert paddle.equal(list(out[0].values())[0], csr[0])
    assert paddle.equal(list(out[1].values())[0].sort()[0], csr[1].sort()[0])

    out = graph_store[key]
    assert paddle.equal(out[0], csr[0])
    assert paddle.equal(out[1].sort()[0], csr[1].sort()[0])
