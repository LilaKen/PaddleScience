...
from paddle_geometric.data import TensorAttr
from paddle_geometric.data.feature_store import AttrView, _FieldStatus
from paddle_geometric.testing import MyFeatureStore
...
def test_feature_store():
    ...
    tensor = paddle.to_tensor([[0.0, 0.0, 0.0], [1.0, 1.0, 1.0], [2.0, 2.0, 2.0]])
    ...
    assert paddle.equal(store.get_tensor(attr), tensor)
    assert paddle.equal(
        store.get_tensor(group_name, attr_name, index=paddle.to_tensor([0, 2])),
        tensor[paddle.to_tensor([0, 2])],
    )

    assert store.update_tensor(tensor + 1, attr)
    assert paddle.equal(store.get_tensor(attr), tensor + 1)
    ...
    assert paddle.equal(store[group_name, attr_name, index], tensor)
    assert paddle.equal(store[group_name, attr_name, None], tensor)
    assert paddle.equal(store[group_name, attr_name, :], tensor)
    assert paddle.equal(store[group_name][attr_name][:], tensor)
    assert paddle.equal(store[group_name].feat[:], tensor)
    assert paddle.equal(store.view().A.feat[:], tensor)
    ...
    assert paddle.equal(store[group_name](), tensor)
...
def test_feature_store_override():
    ...
    assert paddle.equal(store[attr_name, index], tensor)
    assert paddle.equal(store[attr_name][index], tensor)
    assert paddle.equal(store[attr_name][:], tensor)
    assert paddle.equal(store[attr_name, :], tensor)
