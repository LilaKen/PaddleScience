...
def get_temporal_data(num_events, msg_channels):
    return TemporalData(
        src=paddle.arange(num_events),
        dst=paddle.arange(num_events, num_events * 2),
        t=paddle.arange(0, num_events * 1000, step=1000),
        msg=paddle.randn(shape=[num_events, msg_channels]),
        y=paddle.randint(0, 2, (num_events, )),
    )
...
def test_temporal_indexing():
    ...
    index = [0, 4, 8]
    subset = data[paddle.to_tensor(index)]
    assert isinstance(subset, TemporalData)
    assert len(subset) == 3
    ...
    subset = data[paddle.to_tensor(mask)]
    assert isinstance(subset, TemporalData)
    assert len(subset) == 5
