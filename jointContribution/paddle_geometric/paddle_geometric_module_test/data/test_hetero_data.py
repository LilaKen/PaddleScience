...
x_paper = paddle.randn(shape=[10, 16])
x_author = paddle.randn(shape=[5, 32])
x_conference = paddle.randn(shape=[5, 8])

idx_paper = paddle.randint(x_paper.shape[0], (100, ), dtype=paddle.int64)
idx_author = paddle.randint(x_author.shape[0], (100, ), dtype=paddle.int64)
idx_conference = paddle.randint(x_conference.shape[0], (100, ), dtype=paddle.int64)

edge_index_paper_paper = paddle.stack([idx_paper[:50], idx_paper[:50]], dim=0)
edge_index_paper_author = paddle.stack([idx_paper[:30], idx_author[:30]], dim=0)
edge_index_author_paper = paddle.stack([idx_author[:30], idx_paper[:30]], dim=0)
edge_index_paper_conference = paddle.stack(
    [idx_paper[:25], idx_conference[:25]], dim=0)

edge_attr_paper_paper = paddle.randn(shape=[edge_index_paper_paper.shape[1], 8])
edge_attr_author_paper = paddle.randn(shape=[edge_index_author_paper.shape[1], 8])
...
def test_hetero_data_to_from_dict():
    data = HeteroData()
    data.global_id = '1'
    data['v1'].x = paddle.randn(shape=[5, 16])
    data['v2'].y = paddle.randn(shape=[4, 16])
    data['v1', 'v2'].edge_index = paddle.to_tensor([[0, 1, 2, 3], [0, 1, 2, 3]])

    out = HeteroData.from_dict(data.to_dict())
    assert out.global_id == data.global_id
    assert paddle.equal(out['v1'].x, data['v1'].x)
    assert paddle.equal(out['v2'].y, data['v2'].y)
    assert paddle.equal(out['v1', 'v2'].edge_index, data['v1', 'v2'].edge_index)
...
def test_hetero_data_set_value_dict():
    ...
    data.set_value_dict('x', {
        'paper': paddle.randn(shape=[4, 16]),
        'author': paddle.randn(shape=[8, 32]),
    })
    assert data.node_types == ['paper', 'author']
    assert data.edge_types == []
    assert data['paper'].x.shape== (4, 16)
    assert data['author'].x.shape== (8, 32)
...
def test_hetero_data_subgraph():
    ...
    for key in out.node_types:
        node_mask[key] = paddle.zeros((data[key].num_nodes, ), dtype=paddle.bool)
        node_map[key] = paddle.zeros((data[key].num_nodes, ), dtype=paddle.int64)
    ...
    for key in out.edge_types:
        assert len(out[key]) == len(data[key])
        assert paddle.equal(out[key].edge_index, subgraph_edge_index[key])
    ...
    assert out['paper', 'paper'].edge_index.shape== (2, 4)
    assert out['paper', 'paper'].edge_attr.shape== (4, 8)
...
def test_hetero_data_empty_subgraph():
    ...
    out = data.subgraph(subset_dict={
        'paper': paddle.to_tensor([1, 2, 3]),
        'author': paddle.to_tensor([1, 2, 3]),
    })

    assert paddle.equal(out['paper'].x, paddle.arange(1, 4))
    assert out['paper'].num_nodes == 3
    assert paddle.equal(out['author'].x, paddle.arange(1, 4))
    assert out['author'].num_nodes == 3
    assert 'edge_index' not in out['paper', 'author']
    assert paddle.equal(out['paper', 'author'].edge_weight, paddle.arange(5))
...
def test_to_homogeneous_and_vice_versa():
    ...
    data['paper', 'paper'].edge_weight = paddle.randn(shape=[250, ])
    data['paper', 'paper'].edge_attr = paddle.randn(shape=[250, 64])

    data['paper', 'author'].edge_index = get_random_edge_index(100, 200, 500)
    data['paper', 'author'].edge_weight = paddle.randn(shape=[500, ])
    data['paper', 'author'].edge_attr = paddle.randn(shape=[500, 64])

    data['author', 'paper'].edge_index = get_random_edge_index(200, 100, 1000)
    data['author', 'paper'].edge_weight = paddle.randn(shape=[1000, ])
    data['author', 'paper'].edge_attr = paddle.randn(shape=[1000, 64])
...
def test_to_homogeneous_padding():
    data = HeteroData()
    data['paper'].x = paddle.randn(shape=[100, 128])
    data['author'].x = paddle.randn(shape=[50, 64])

    out = data.to_homogeneous()
    assert len(out) == 2
    assert out.node_type.shape== (150, )
    assert out.node_type[:100].abs().sum() == 0
    assert out.node_type[100:].sub(1).abs().sum() == 0
    assert out.x.shape== (150, 128)
    assert paddle.equal(out.x[:100], data['paper'].x)
    assert paddle.equal(out.x[100:, :64], data['author'].x)
...
def test_hetero_data_invalid_names():
    ...
    with pytest.warns(UserWarning, match="single underscores"):
        data['my test', 'a__b', 'my test'].edge_attr = paddle.randn(shape=[10, 16])
    with warnings.catch_warnings():  # No warning should be raised afterwards:
        warnings.simplefilter('error')
        data['my test', 'a__c', 'my test'].edge_attr = paddle.randn(shape=[10, 16])
    assert data.edge_types == [
...
def test_hetero_data_update():
    ...
    other['paper', 'paper'].edge_index = paddle.randint(5, (2, 20))

    data.update(other)
    assert len(data) == 3
    assert paddle.equal(data['paper'].x, paddle.arange(15, 20))
    assert paddle.equal(data['paper'].y, paddle.arange(5, 10))
    assert paddle.equal(data['author'].x, paddle.arange(10, 15))
    assert paddle.equal(data['author'].y, paddle.arange(20, 25))
    assert paddle.equal(data['paper', 'paper'].edge_index,
...
def test_basic_feature_store():
    ...
    assert data.put_tensor(copy.deepcopy(x), group_name='paper', attr_name='x',
                           index=None)
    assert paddle.equal(data['paper'].x, x)
    ...
    data['paper'].bad_attr = paddle.randn(shape=[10, 20])  # don't include, bad cat_dim
...
def test_hetero_data_with_tensor_frame():
    ...
    # Test subgraph:
    subset = {
        'paper': paddle.to_tensor([1, 2, 3, 4]),
        'author': paddle.to_tensor([0, 1, 2, 3]),
    }
    out = data.subgraph(subset)
...
def test_basic_graph_store():
    ...
    def assert_equal_tensor_tuple(expected, actual):
        ...
        for i in range(len(expected)):
            assert paddle.equal(expected[i], actual[i])
...
def test_invalid_keys():
    ...
    data['paper'].x = paddle.randn(shape=[10, 128])
