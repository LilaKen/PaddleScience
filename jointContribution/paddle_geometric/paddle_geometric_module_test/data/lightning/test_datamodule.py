...
from paddle_geometric.data.lightning import (
    LightningDataset,
    LightningLinkData,
    LightningNodeData,
)
from paddle_geometric.nn import global_mean_pool
...
class LinearGraphModule(LightningModule):
    def __init__(self, in_channels: int, hidden_channels: int,
        ...
        self.lin1 = paddle.nn.Linear(in_channels, hidden_channels)
        self.lin2 = paddle.nn.Linear(hidden_channels, out_channels)
    ...
    def configure_optimizers(self):
        return paddle.optim.Adam(self.parameters(), lr=0.01)
...
def test_lightning_dataset(get_dataset, strategy_type):
    ...
    devices = 1 if strategy_type is None else paddle.cuda.device_count()
    if strategy_type == 'ddp':
        strategy = pl.strategies.DDPStrategy(accelerator='gpu')
    else:
        strategy = pl.strategies.SingleDeviceStrategy(place='gpu:0')
    ...
    assert paddle.all(new_x > old_x)  # Ensure shared data.
...
class LinearNodeModule(LightningModule):
    ...
    def training_step(self, data: Data, batch_idx: int):
        ...
        self.log('loss', loss, batch_size=y.shape[0])
        self.log('train_acc', self.train_acc, batch_size=y.shape[0])
        return loss
    ...
    def validation_step(self, data: Data, batch_idx: int):
        ...
        self.log('val_acc', self.val_acc, batch_size=y.shape[0])

    def test_step(self, data: Data, batch_idx: int):
        y_hat = self(data.x)[data.test_mask]
        y = data.y[data.test_mask]
        self.test_acc(y_hat.softmax(dim=-1), y)
        self.log('test_acc', self.test_acc, batch_size=y.shape[0])
...
def preserve_context():
    num_threads = paddle.get_num_threads()
    yield
    if paddle.distributed.is_initialized():
        paddle.distributed.destroy_process_group()
    paddle.set_num_threads(num_threads)
...
def test_lightning_hetero_node_data(preserve_context, get_dataset):
    ...
    devices = paddle.cuda.device_count()
    strategy = pl.strategies.DDPStrategy(accelerator='gpu')
    ...
    trainer.fit(model, datamodule)
    trainer.test(model, datamodule)
    assert paddle.all(data['paper'].x > original_x)  # Ensure shared data.
    assert trainer.validate_loop._data_source.is_defined()
    assert trainer.test_loop._data_source.is_defined()
...
def test_lightning_hetero_link_data():
    ...
    data['paper'].x = paddle.arange(10)
    data['author'].x = paddle.arange(10)
    data['term'].x = paddle.arange(10)
    ...
    data['author'].time = paddle.arange(data['author'].num_nodes)
    data['paper'].time = paddle.arange(data['paper'].num_nodes)
    data['term'].time = paddle.arange(data['term'].num_nodes)

    datamodule = LightningLinkData(
        data,
        input_train_edges=('author', 'paper'),
        input_train_time=paddle.arange(data['author', 'paper'].num_edges),
        loader='neighbor',
        num_neighbors=[5],
        batch_size=32,
        num_workers=0,
