...
def test_base_storage():
    ...
    storage.x = paddle.zeros(1)
    storage.y = paddle.ones(1)
    assert len(storage) == 2
    assert storage._mapping == {'x': paddle.zeros(1), 'y': paddle.ones(1)}
    ...
    storage = BaseStorage({'x': paddle.zeros(1)})
    assert len(storage) == 1
    assert storage.x is not None
...
def test_storage_tensor_methods():
    ...
    assert storage.x.device == paddle.place('cpu')
