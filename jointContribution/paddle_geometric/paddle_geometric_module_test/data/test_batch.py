...
from paddle_geometric.data import Batch, Data, HeteroData
from paddle_geometric.testing import get_random_edge_index, withPackage
from paddle_geometric.typing import SparseTensor
from paddle_geometric.utils import to_edge_index, to_paddle_sparse_tensor
...
def test_batch_basic():
    ...
    assert len(batch.index_select(paddle.to_tensor([1, 0]))) == 2
    assert len(batch.index_select(paddle.to_tensor([True, False]))) == 1
...
def test_index():
    ...
    assert batch.index.equal(paddle.to_tensor([0, 1, 1, 2, 3, 4, 4, 5, 5, 6]))
...
def test_batch_with_sparse_tensor():
    x = SparseTensor.from_dense(paddle.to_tensor([[1.0], [2.0], [3.0]]))
    edge_index = paddle.to_tensor([[0, 1, 1, 2], [1, 0, 2, 1]])
    adj = SparseTensor.from_edge_index(edge_index)
    data1 = Data(x=x, adj=adj)

    x = SparseTensor.from_dense(paddle.to_tensor([[1.0], [2.0]]))
    edge_index = paddle.to_tensor([[0, 1], [1, 0]])
    adj = SparseTensor.from_edge_index(edge_index)
    data2 = Data(x=x, adj=adj)

    x = SparseTensor.from_dense(paddle.to_tensor([[1.0], [2.0], [3.0], [4.0]]))
    edge_index = paddle.to_tensor([[0, 1, 1, 2, 2, 3], [1, 0, 2, 1, 3, 2]])
    adj = SparseTensor.from_edge_index(edge_index)
...
def test_batching_with_new_dimension():
    ...
    x1 = paddle.to_tensor([1, 2, 3], dtype=paddle.float32)
    foo1 = paddle.randn(shape=[4])
    y1 = paddle.to_tensor(1)

    x2 = paddle.to_tensor([1, 2], dtype=paddle.float32)
    foo2 = paddle.randn(shape=[4])
    y2 = paddle.to_tensor(2)
...
def test_pickling(tmp_path):
    data = Data(x=paddle.randn(shape=[5, 16]))
    ...
    path = osp.join(tmp_path, 'batch.pdparams')
...
def test_recursive_batch():
    data1 = Data(
        x={
            '1': paddle.randn(shape=[10, 32]),
            '2': paddle.randn(shape=[20, 48])
        },
        edge_index=[
    ...
    data2 = Data(
        x={
            '1': paddle.randn(shape=[20, 32]),
            '2': paddle.randn(shape=[40, 48])
        },
        edge_index=[
    ...
    assert paddle.allclose(batch.x['1'],
                          paddle.concat([data1.x['1'], data2.x['1']], dim=0))
    assert paddle.allclose(batch.x['2'],
                          paddle.concat([data1.x['2'], data2.x['2']], dim=0))
    assert (batch.edge_index[0].tolist() == paddle.concat(
        [data1.edge_index[0], data2.edge_index[0] + 30], dim=1).tolist())
    assert (batch.edge_index[1].tolist() == paddle.concat(
        [data1.edge_index[1], data2.edge_index[1] + 30], dim=1).tolist())
    assert batch.batch.shape== (90, )
    assert batch.ptr.shape== (3, )

    out1 = batch[0]
    assert len(out1) == 3
    assert out1.num_nodes == 30
    assert paddle.allclose(out1.x['1'], data1.x['1'])
    assert paddle.allclose(out1.x['2'], data1.x['2'])
    ...
    assert paddle.allclose(out2.x['1'], data2.x['1'])
    assert paddle.allclose(out2.x['2'], data2.x['2'])
...
def test_batching_of_batches():
    data = Data(x=paddle.randn(shape=[2, 16]))
...
def test_hetero_batch():
    ...
    e2 = ('a', 'p')
    data1 = HeteroData()
    data1['p'].x = paddle.randn(shape=[100, 128])
    data1['a'].x = paddle.randn(shape=[200, 128])
    data1[e1].edge_index = get_random_edge_index(100, 200, 500)
    data1[e1].edge_attr = paddle.randn(shape=[500, 32])
    data1[e2].edge_index = get_random_edge_index(200, 100, 400)
    data1[e2].edge_attr = paddle.randn(shape=[400, 32])

    data2 = HeteroData()
    data2['p'].x = paddle.randn(shape=[50, 128])
    data2['a'].x = paddle.randn(shape=[100, 128])
    data2[e1].edge_index = get_random_edge_index(50, 100, 300)
    data2[e1].edge_attr = paddle.randn(shape=[300, 32])
    data2[e2].edge_index = get_random_edge_index(100, 50, 200)
    data2[e2].edge_attr = paddle.randn(shape=[200, 32])
    ...
    assert paddle.allclose(batch['p'].x[:100], data1['p'].x)
    assert paddle.allclose(batch['a'].x[:200], data1['a'].x)
    assert paddle.allclose(batch['p'].x[100:], data2['p'].x)
    assert paddle.allclose(batch['a'].x[200:], data2['a'].x)
    assert (batch[e1].edge_index.tolist() == paddle.concat([
        data1[e1].edge_index,
        data2[e1].edge_index + paddle.to_tensor([[100], [200]])
    ], 1).tolist())
    assert paddle.allclose(
        batch[e1].edge_attr,
        paddle.concat([data1[e1].edge_attr, data2[e1].edge_attr], 0))
    assert (batch[e2].edge_index.tolist() == paddle.concat([
        data1[e2].edge_index,
        data2[e2].edge_index + paddle.to_tensor([[200], [100]])
    ], 1).tolist())
    assert paddle.allclose(
        batch[e2].edge_attr,
        paddle.concat([data1[e2].edge_attr, data2[e2].edge_attr], 0))
    assert batch['p'].batch.shape== (150, )
    assert batch['p'].ptr.shape== (3, )
    assert batch['a'].batch.shape== (300, )
    assert batch['a'].ptr.shape== (3, )

    out1 = batch[0]
    assert len(out1) == 3
    assert out1.num_nodes == 300
    assert paddle.allclose(out1['p'].x, data1['p'].x)
    assert paddle.allclose(out1['a'].x, data1['a'].x)
    assert out1[e1].edge_index.tolist() == data1[e1].edge_index.tolist()
    assert paddle.allclose(out1[e1].edge_attr, data1[e1].edge_attr)
    assert out1[e2].edge_index.tolist() == data1[e2].edge_index.tolist()
    assert paddle.allclose(out1[e2].edge_attr, data1[e2].edge_attr)

    out2 = batch[1]
    assert len(out2) == 3
    assert out2.num_nodes == 150
    assert paddle.allclose(out2['p'].x, data2['p'].x)
    assert paddle.allclose(out2['a'].x, data2['a'].x)
    assert out2[e1].edge_index.tolist() == data2[e1].edge_index.tolist()
    assert paddle.allclose(out2[e1].edge_attr, data2[e1].edge_attr)
    assert out2[e2].edge_index.tolist() == data2[e2].edge_index.tolist()
    assert paddle.allclose(out2[e2].edge_attr, data2[e2].edge_attr)
...
def test_pair_data_batching():
    ...
    x_s = paddle.randn(shape=[5, 16])
    edge_index_s = paddle.to_tensor([
        [0, 0, 0, 0],
        [1, 2, 3, 4],
    ])
    x_t = paddle.randn(shape=[4, 16])
    edge_index_t = paddle.to_tensor([
        [0, 0, 0],
        [1, 2, 3],
    ])
    ...
    assert paddle.allclose(batch.x_s, paddle.concat([x_s, x_s], dim=0))
    assert batch.edge_index_s.tolist() == [[0, 0, 0, 0, 5, 5, 5, 5],
                                           [1, 2, 3, 4, 6, 7, 8, 9]]

    assert paddle.allclose(batch.x_t, paddle.concat([x_t, x_t], dim=0))
...
SPARSE_LAYOUTS: List[Union[str, Exception]] =  [
    'paddle_sparse',
    pytest.param(
        'paddle.sparse_csc',
        marks=pytest.mark.skip(reason="paddle.sparse_csc is not implemented in Paddle.")
    ),
    pytest.param(
        'paddle.sparse_coo',
        marks=pytest.mark.skip(reason="paddle.sparse_coo is not implemented in Paddle.")
    ),
    pytest.param(
        'paddle.sparse_csr',
        marks=pytest.mark.skip(reason="paddle.sparse_csr is not implemented in Paddle.")
    ),
]

@pytest.mark.parametrize('layout', SPARSE_LAYOUTS)
def test_paddle_sparse_batch(layout):
    x_dense = paddle.randn(shape=[3, 4])
    x = x_dense.to_sparse(layout=layout)
    edge_index = paddle.to_tensor([[0, 1, 1, 2], [1, 0, 2, 1]])
    edge_attr = paddle.rand(4)
    adj = to_paddle_sparse_tensor(edge_index, edge_attr, layout=layout)

    data = Data(x=x, adj=adj)

    batch = Batch.from_data_list([data, data])

    assert batch.x.shape== (6, 4)
    assert batch.x.layout in {paddle.sparse_coo, paddle.sparse_csr}
    assert paddle.equal(batch.x.to_dense(), paddle.concat([x_dense, x_dense], 0))

    assert batch.adj.shape== (6, 6)
    assert batch.adj.layout == layout
    out = to_edge_index(batch.adj.to_sparse(layout=paddle.sparse_csr))
    assert paddle.equal(out[0], paddle.concat([edge_index, edge_index + 3], 1))
    assert paddle.equal(out[1], paddle.concat([edge_attr, edge_attr], 0))


def test_paddle_nested_batch():
    from paddle.nested import nested_tensor

    class MyData(Data):
        def __inc__(self, key, value, *args, **kwargs) -> int:
            return 2

    x1 = nested_tensor([paddle.randn(shape=[3]), paddle.randn(shape=[4])])
    data1 = MyData(x=x1)
    assert str(data1) == 'MyData(x=[2, 4])'

    x2 = nested_tensor([paddle.randn(shape=[3]), paddle.randn(shape=[4]), paddle.randn(shape=[5])])
    data2 = MyData(x=x2)
