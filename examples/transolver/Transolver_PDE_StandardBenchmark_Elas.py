...
def train(cfg: DictConfig):
    ...
    # set model
    model = ppsci.arch.Transolver_Irregular_Mesh_Elas(
    ...
    elasticity_constraint = ppsci.constraint.SupervisedConstraint(
        train_dataloader_cfg,
        ppsci.loss.L2RelLoss("mean"),
        name="Elasticity_constraint",
    )

    constraint = {elasticity_constraint.name: elasticity_constraint}
    ...
    elasticity_valid = ppsci.validate.SupervisedValidator(
        valid_dataloader_cfg,
        loss=ppsci.loss.L2RelLoss("mean"),
        metric={"MeanL2Rel": ppsci.metric.MeanL2Rel()},
        name="Elasticity_valid",
    )

    validator = {elasticity_valid.name: elasticity_valid}
...
@hydra.main(version_base=None, config_path="./conf", config_name="Transolver_PDE_StandardBenchmark_Elas.yaml")
