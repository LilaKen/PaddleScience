...
def train(cfg: DictConfig):
    ...
    model = ppsci.arch.Transolver_Structured_Mesh_2D(
        ...
        H=int((cfg.TRAIN.s1 - 1) / cfg.TRAIN.r1 + 1),
        W=int((cfg.TRAIN.s2 - 1) / cfg.TRAIN.r2 + 1),
    )
    ...
    train_dataloader_cfg = {
        "dataset": {
            ...
            "s2": cfg.Train.s2,
            "T": cfg.Train.T,
            "Deformation": cfg.Train.Deformation,
            "r1": cfg.Train.r1,
            "r2": cfg.Train.r2,
    ...
    plas_constraint = ppsci.constraint.SupervisedConstraint(
        train_dataloader_cfg,
        ppsci.loss.L2RelLoss(),
        name="Plas_constraint",
    )

    constraint = {plas_constraint.name: plas_constraint}
    ...
    plas_valid = ppsci.validate.SupervisedValidator(
        valid_dataloader_cfg,
    ...
    validator = {plas_valid.name: plas_valid}
    ...
    optimizer = ppsci.optimizer.Adam(lr_scheduler, weight_decay=cfg.TRAIN.weight_decay)(model)
...
def evaluate(cfg: DictConfig):
    ...
    model = ppsci.arch.Transolver_Structured_Mesh_2D(
        ...
        ref=cfg.MODEL.ref,
        unified_pos=cfg.MODEL.unified_pos,
        H=int((cfg.EVAL.s1 - 1) / cfg.EVAL.r1 + 1),
        W=int((cfg.EVAL.s2 - 1) / cfg.EVAL.r2 + 1),
    )
    ...
    valid_dataloader_cfg = {
        "dataset": {
            ...
            "s2": cfg.EVAL.s2,
            "T": cfg.EVAL.T,
            "Deformation": cfg.EVAL.Deformation,
            "r1": cfg.EVAL.r1,
            "r2": cfg.EVAL.r2,
            "mode": cfg.EVAL.mode,
        },
...
@hydra.main(version_base=None, config_path="./conf", config_name="Transolver_PDE_StandardBenchmark_Plas.yaml")
