...
def train(cfg: DictConfig):
    ...
    model = ppsci.arch.Transolver_Structured_Mesh_2D(
        ...
        H=int((129 - 1) / cfg.TRAIN.r1 + 1),
        W=int((129 - 1) / cfg.TRAIN.r2 + 1),
        task="Pipe",
    )
    ...
    pipe_constraint = ppsci.constraint.SupervisedConstraint(
        train_dataloader_cfg,
        ppsci.loss.L2RelLoss("mean"),
        name="Pipe_constraint",
    )

    constraint = {pipe_constraint.name: pipe_constraint}
    ...
    pipe_valid = ppsci.validate.SupervisedValidator(
        valid_dataloader_cfg,
    ...
    validator = {pipe_valid.name: pipe_valid}
...
def evaluate(cfg: DictConfig):
    ...
    model = ppsci.arch.Transolver_Structured_Mesh_2D(
        ...
        ref=cfg.MODEL.ref,
        unified_pos=cfg.MODEL.unified_pos,
        H=int((129 - 1) / cfg.EVAL.r1 + 1),
        W=int((129 - 1) / cfg.EVAL.r2 + 1),
        task="Pipe",
    )
...
@hydra.main(version_base=None, config_path="./conf", config_name="Transolver_PDE_StandardBenchmark_Pipe.yaml")
