...
def train(cfg: DictConfig):
    ...
    model = ppsci.arch.Transolver_Structured_Mesh_2D(
        ...
        H=int((421 - 1) / cfg.TRAIN.r + 1),
        W=int((421 - 1) / cfg.TRAIN.r + 1),
        task="Darcy",
    )
    ...
    train_dataloader_cfg = {
        "dataset": {
            "name": "DarcyDataset",
            "train_path": cfg.TRAIN.train_path,
            "eval_path": cfg.TRAIN.test_path,
            "input_keys": ("input",),
            "label_keys": ("label",),
            "weight_keys": ("label",),
    ...
    darcy_constraint = ppsci.constraint.SupervisedConstraint(
        train_dataloader_cfg,
        ppsci.loss.L2RelLoss("mean"),
        name="Darcy_constraint",
    )

    constraint = {darcy_constraint.name: darcy_constraint}
    ...
    darcy_valid = ppsci.validate.SupervisedValidator(
        valid_dataloader_cfg,
    ...
    validator = {darcy_valid.name: darcy_valid}
...
def evaluate(cfg: DictConfig):
    ...
    valid_dataloader_cfg = {
        "dataset": {
            "name": "DarcyDataset",
            "train_path": cfg.EVAL.train_path,
            "eval_path": cfg.EVAL.test_path,
            "input_keys": ("input",),
            "label_keys": ("label",),
            "weight_keys": ("label",),
...
@hydra.main(version_base=None, config_path="./conf", config_name="Transolver_PDE_StandardBenchmark_Darcy.yaml")
